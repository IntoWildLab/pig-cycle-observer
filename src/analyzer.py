...
"""
    ...
    @staticmethod
    def _is_deepseek_model(model: str, provider: Optional[str] = None) -> bool:
        """Return whether an OpenAI-compatible route targets DeepSeek."""
        return "deepseek" in str(model).lower() or str(provider or "").lower() == "deepseek"
    ...
    def _log_completion_diagnostics(
        ...
        *,
        ...
        """Log metadata for an LLM response without logging response content."""
        choices = self._get_response_field(response, "choices") if response is not None else None
        choice = choices[0] if choices else None
        message = self._get_response_field(choice, "message") if choice is not None else None
        reasoning_content = (
            self._get_response_field(message, "reasoning_content")
            if message is not None
            else None
        )
        usage = self._get_response_field(response, "usage") if response is not None else None
        logger.info(
            "[LLM诊断] model=%s attempt=%s call_success=%s choices_exists=%s "
            "message_content_empty=%s reasoning_content_exists=%s finish_reason=%s "
            ...
            reasoning_content is not None and bool(str(reasoning_content).strip()),
            self._get_response_field(choice, "finish_reason") if choice is not None else None,
            self._get_response_field(usage, "prompt_tokens") if usage is not None else None,
            self._get_response_field(usage, "completion_tokens") if usage is not None else None,
            self._get_response_field(usage, "total_tokens") if usage is not None else None,
        )

    def _call_nonstream_with_deepseek_empty_retry(
        self,
        *,
        model: str,
        ...
        """Retry one successful-but-empty DeepSeek response with thinking disabled."""
        response = completion_callable(call_kwargs)
        content = self._extract_completion_text(response)
        self._log_completion_diagnostics(
            response,
            model=model,
            attempt="initial",
            content=content,
        )
        if response is None or content or not self._is_deepseek_model(model, provider):
            return response, content

        time.sleep(3)
        retry_kwargs = dict(call_kwargs)
        retry_extra_body = dict(retry_kwargs.get("extra_body") or {})
        retry_extra_body["thinking"] = {"type": "disabled"}
        retry_kwargs["extra_body"] = retry_extra_body

        retry_response = completion_callable(retry_kwargs)
        retry_content = self._extract_completion_text(retry_response)
        self._log_completion_diagnostics(
            retry_response,
            model=model,
            attempt="empty_response_retry",
            content=retry_content,
        )
        return retry_response, retry_content
        ...
        *,
        ...
        try:
            for chunk in stream_response:
                chunk_choices = self._get_response_field(chunk, "choices")
                if chunk_choices:
                    choices_seen = True
                    chunk_choice = chunk_choices[0]
                    chunk_delta = self._get_response_field(chunk_choice, "delta")
                    chunk_message = self._get_response_field(chunk_choice, "message")
                    ...
                    if reasoning_content is None and chunk_message is not None:
                        reasoning_content = self._get_response_field(
                            chunk_message,
                            "reasoning_content",
                        )
                    ...
                    chunk_finish_reason = self._get_response_field(chunk_choice, "finish_reason")
        ...
        diagnostic_response = {
            "choices": ([{
                "message": {
                    ...
                    "reasoning_content": "present" if reasoning_content_seen else None,
                },
                "finish_reason": finish_reason,
            }] if choices_seen else []),
            "usage": usage,
        }
        self._log_completion_diagnostics(
            diagnostic_response,
            model=model,
            attempt="initial_stream",
            content=response_text,
        )
        ...
        *,
        ...
        for model in models_to_try:
            ...
            try:
                ...
                response, content = self._call_nonstream_with_deepseek_empty_retry(
                    model=model,
                    provider=usage_provider,
                    call_kwargs=call_kwargs,
                    completion_callable=lambda kwargs: call_litellm_with_param_recovery(
                        lambda recovered_kwargs: self._dispatch_litellm_completion(
                            model,
                            recovered_kwargs,
                            config=config,
                            use_channel_router=use_channel_router,
